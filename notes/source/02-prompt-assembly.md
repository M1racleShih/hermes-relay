...
Skills 是 prompt 中最重的部分（数百个文件），采用**两层缓存 + 全量扫描 fallback**（进程内 LRU → 磁盘 Snapshot → miss 后全量扫描）来优化启动性能。Skill 的运行时加载（`/skill` 命令）作为 **USER message** 注入对话，而非写入 system prompt，从而保持前缀缓存稳定。
...
2. **`agent/prompt_builder.py:988` `build_skills_system_prompt()`** → 理解 skills 两层缓存、fallback 扫描和过滤流水线，这是最复杂的函数
...
| 3 | Skills 缓存与 fallback 扫描流水线 | `prompt_builder.py:988-1219` | ★★★ | LRU(8 slots) → 磁盘 Snapshot(mtime+size manifest) → miss 后全量扫描，需要理解何时命中、何时失效 |
...
这张图回答 prompt assembly 的边界问题：`AIAgent` 负责把 stable/context/volatile 三组 prompt parts 合并并缓存，`prompt_builder.py` 负责其中的 context files、skills index、environment hints 等具体组件。读图时重点看 `_cached_system_prompt`，它解释了为什么 mid-session 的变化不会自动重写 system prompt。
...
这张图解释项目上下文文件的优先级。它不是“全部加载”，而是按 `.hermes.md`、`AGENTS.md`、`CLAUDE.md`、`.cursorrules` 的顺序选择第一个可用内容，再按 `skip_soul` 决定是否追加 SOUL。
...
这张图解释 skills index 如何进入 system prompt。注意这里进入的是“索引”（名称、描述、过滤后的可见性），不是完整 `SKILL.md` 内容；完整内容要等 `/skill` 命令运行时再作为 user message 注入。
...
### 3.2 两层缓存 + 全量扫描 fallback

这张图解释 `build_skills_system_prompt()` 的性能策略：先查进程内 LRU，再查磁盘 snapshot；两者都 miss 时才全量扫描技能目录并回写缓存。全量扫描是 fallback 路径，不是长期缓存层。
...
graph TD
    ...
    SNAP -->|"不匹配"| SCAN["Fallback: 全量扫描<br/>遍历所有 SKILL.md<br/>解析 frontmatter"]
...
这张图解释 memory 写入为什么不会立即改变 system prompt。关键点是 `_system_prompt_snapshot` 在会话启动时冻结，之后 memory tool 写磁盘和 live state，但下一轮 API 调用仍读取 frozen snapshot。
...
这张图说明 MemoryManager 的职责边界：内置 memory provider 负责本地 `MEMORY.md` / `USER.md`，外部 provider 作为可选插件接入，tool 调用通过 MemoryManager 路由。
...
| `agent/skill_utils.py` | 511 | Skill 发现、frontmatter 解析、平台过滤 |

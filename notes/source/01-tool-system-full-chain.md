...
这张图回答 Tool System 的边界问题：AIAgent 不直接面对每个工具文件，而是通过 `model_tools.py` 统一拿 schema、处理 tool call、再交给 registry dispatch。红色虚线的 agent-level tools 是例外，它们在 agent loop 内部拦截，不走普通 registry dispatch。
...
这张 class diagram 只保留理解 registry 行为必须知道的字段。读它的重点不是字段数量，而是 `schema`、`handler`、`check_fn` 和 `dynamic_schema_overrides` 分别服务于“模型看到什么”“实际执行什么”“是否暴露”“运行时如何修正 schema”。
...
这张图把 tool 的生命周期分成三段：进程启动时注册、API 调用前生成 schema、LLM 返回 tool call 后执行。它的核心结论是：注册和执行不是一回事，`check_fn` 只影响 schema 暴露，真正的安全检查发生在 handler 内部。
...
这张图解释 terminal command 的执行前 guard 顺序。需要特别注意两点：容器环境会提前放行；hardline 和 sudo stdin guard 在 yolo/smart/off 之前执行，因此不能被用户偏好绕过。
...
这张图解释 toolset 名称如何展开成具体 tool 名称。普通 toolset 查 `TOOLSETS`，组合 toolset 递归展开，`hermes-*` 平台 preset 会走平台注册逻辑；未知名称安全地返回空列表。
...
这张图解释 `_run_async()` 为什么不能简单地到处 `asyncio.run()`。不同调用上下文需要不同 event loop 策略，否则缓存的 async client 可能在 loop 销毁后触发 `Event loop is closed`。

...
3. 遇到流程性内容，优先在对应的 `notes/source/` 或 `notes/design/` 主题笔记中内嵌 Mermaid 图表达。
...
当用户要求 agent 在休息或离线时按阶段推进学习计划时，按下面规则执行：
...
6. 涉及流程、状态机、协议映射或调用链时，必须在当前主题笔记中写 Mermaid 图；不要把 Mermaid 当成独立知识分类。
...
- 阅读源码：3-6 个紧密相关文件；
- 新增笔记：1-2 份；
- Mermaid 图：1-2 张，内嵌在 `notes/source/` 或 `notes/design/`；
- 源码改动：默认 0；

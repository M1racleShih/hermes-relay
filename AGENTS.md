...
5. 每次学习会话都要留下可追踪证据：优先新增或完善 `notes/source/` / `notes/design/`，并用 commit message 记录目标、变更和验证；只有需要保留探索过程、疑问或接续点时才写短 journal。
...
5. 每个切片至少新增或完善一份 `notes/source/` 或 `notes/design/`；如果 commit message 已完整记录目标、变更、验证和接续点，journal 可以省略。
6. 涉及复杂流程、状态机、协议映射或调用链时，自然选择合适的表达方式；如果用了图，图前后都要有文字解释。
7. 每个切片必须执行一个验证动作：可以是只读脚本、现有测试命令、fixture/日志检查、或明确的测试设计；验证是类似 lint/test 的工作步骤，不作为笔记评分项，结果写在最终汇报或 commit message，不写进 `notes/source/` 或 `notes/design/` 正文。
